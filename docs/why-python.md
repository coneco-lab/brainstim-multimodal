...
- Matrix-friendliness: MATLAB makes it easy to work with matrices, and neuroscience data are usually just that. For example, behavioural data are usually a table with one row per subject and one column per variable, while EEG data are matrices with one row per channel and column per timepoint
